...
2. **Notebook 01** (`kaggle/01_data_tokenizers.ipynb`, CPU, Internet On, gắn `vitok-code`):
...
4. **Notebook 02** (`kaggle/02_train_eval.ipynb`, GPU T4 ×2, gắn `vitok-data` + `vitok-code`):
   ...
   Kết quả nằm ở `/kaggle/working/results/*.json`; tải về thư mục `results/` của repo.
